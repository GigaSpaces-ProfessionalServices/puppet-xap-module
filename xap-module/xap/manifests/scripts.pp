class xap::scripts (
  ...
  $manager_stabilization_delay = $xap::params::manager_stabilization_delay,
  ...
  if $nodetype == 'manager' {
    ...
    file { "$install_dir/scripts/xap-mgr.sh":
      ...
      owner   => $owner_user,
      mode    => '0775',
      notify  => Exec['sanitize xap-mgr.sh'],
    }

    exec { "sanitize xap-mgr.sh":
      environment => $env_vars,
      command     => "/bin/sed -i -e 's/\r$//' -e '/^\s*$/d' xap-mgr.sh",
    ...
    file { "$install_dir/scripts/xap-non-mgr.sh":
      ...
      owner   => $owner_user,
      mode    => '0775',
      notify  => Exec['sanitize xap-non-mgr.sh'],
    }

    exec { "sanitize xap-non-mgr.sh":
      environment => $env_vars,
      command     => "/bin/sed -i -e 's/\r$//' -e '/^\s*$/d' xap-non-mgr.sh",
      cwd         => "$install_dir/scripts",
      user        => $owner_user,
      timeout     => 0,
      # logoutput   => true,
    ...
    exec { "run xap-non-mgr.sh": 
      environment => $env_vars,
      command     => '/bin/bash xap-non-mgr.sh',
      cwd         => "$install_dir/scripts",
      user        => $owner_user,
      timeout     => 0,
      refreshonly => true,
  ...
  exec { "sanitize gs-agent-zone.sh":
    environment => $env_vars,
    command     => "/bin/sed -i -e 's/\r$//' -e '/^\s*$/d' gs-agent-zone.sh",
    cwd         => "$install_dir/scripts",
    user        => $owner_user,
    timeout     => 0,
    # logoutput   => true,
  ...
  exec { "run gs-agent-zone.sh":
    environment => $env_vars,
    command     => "/bin/bash gs-agent-zone.sh",
    cwd         => "$install_dir/scripts",
    user        => $owner_user,
    timeout     => 0,
    refreshonly => true,
